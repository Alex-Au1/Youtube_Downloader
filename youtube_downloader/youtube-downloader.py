...
class NestedRadioButton():
    ...
    def download_video(self, folder):
        ...
        #downloads the video
            ...
            app.loading_page.check_running_thread(video_download_thread, app.download_finish_sc, [root, app.loading_page, self.page_found], "download", download_video.get_progress)
...
class Application(Frame):
    ...
    def search_video(self, entry, current_sc, dl_pg_key=None):
        ...
        valid_youtube_link = download_video.valid_yt_link(search)
    ...
    def choose_download(self, meta, video, video_att, page_found, is_link, prev_page):
        if (not is_link):
            ...
            link = video_att["link"]
            meta_data_thread
            time = format_display.format_time(meta["duration"])
            title = meta["title"]
        ...
        resized_image = format_display.process_image(thumbnail, img_w, img_h)
        ...
        available_formats = download_video.get_available_formats(download_formats)
        video_codes  = [download_video.YtDownloadFormat.mp4_144p.value, download_video.YtDownloadFormat.mp4_240p.value,
                        download_video.YtDownloadFormat.mp4_360p.value, download_video.YtDownloadFormat.mp4_480p.value,
                        download_video.YtDownloadFormat.mp4_720p.value, download_video.YtDownloadFormat.mp4_1080p.value,
                        download_video.YtDownloadFormat.mp4_640x360.value, download_video.YtDownloadFormat.mp4_1280x720.value,
                        download_video.YtDownloadFormat.gp3_176x144.value, download_video.YtDownloadFormat.gp3_320x240.value,
                        download_video.YtDownloadFormat.flv.value]
        ...
        video_formats = download_video.format_filetype(avail_video_formats)

        #audio formats to download video
        available_audio_formats = {download_video.audio_filetypes["mp3"]:None, download_video.audio_filetypes["m4a"]:None, "ogg":None, download_video.audio_filetypes["aac"]:None, download_video.audio_filetypes["wav"]:None, "don't care":None}
    ...
    def entry_change_setting(self,entry_frame,key, entry, var):
        value = entry.get()
        formatted_value = format_display.format_settings(key, value)
